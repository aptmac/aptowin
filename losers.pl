...
my $numgames = {"unranked" => {"top"	 => 0,
		...
		foreach $file(@files)
		{
			...
			else
			{
				...
				foreach(@players)
				{
					#Get information from players that lost the game
					if ($_ =~ m/"winner":false/)
					{
	...
	"{
		...
		print $numgamesJson;
		open (my $fh, '>', "./data/$patch$region\_num_losers.json") or die $!;
	...
	"{
		...
		print $combinedJson,"\n";
		open (my $fh2, '>', "./data/$patch$region\_items_losers.json") or die $!;
